...
public:
    ...
    vector<Contact> filter(int scelta) {
        ...
        switch (scelta) {
            case 1: 
                ...
                cout << endl << "Inserire la lettera iniziale per cui applicare il filtro: ";
                cin >> letteraIniziale;
            ...
            case 2: 
                int numElementi;
                cout << endl << "Inserisci il numero di elementi per il filtro: ";
                cin >> numElementi;
    ...
    friend ostream& operator<<(ostream& out, const PhoneBook& phoneBook) {
        string nomeDefault = "Marco";
        string cognomeDefault = "Rossi";
        string numTelefonoDefault = "123456789";

        for (const auto& contatto : phoneBook.contatti) {
            if (contatto.nome == nomeDefault && contatto.cognome == cognomeDefault && contatto.numTelefono == numTelefonoDefault) {
                out << "Contatto di Default: " << endl;
            }
            out << "Nome: " << contatto.nome << endl << "Cognome: " << contatto.cognome << endl << "Numero di Telefono: " << contatto.numTelefono << endl << "----------------------------------" << endl;
        }
        return out;
    }
};
...
int main() {
    ...
    //Contatto di Default
    rubrica.append({"Marco", "Rossi", "123456789"});

    // Aggiungo contatti
    Contact newContatto;

    do {
        cout << endl << "Vuoi inserire un nuovo contatto? (1-si, 2-no): ";
        cin >> scelta;
        if (scelta == 1) {
            cout << "Nome: ";
    ...
    cout << endl << "Quale contatto vuoi ricercare? (Inserisci il nome): ";
    cin >> nomeCercato;

    Contact* contattoTrovato = rubrica.find(nomeCercato);
    if (contattoTrovato != nullptr) {
        cout << "[OK] Contatto trovato: " << contattoTrovato->nome << " " << contattoTrovato->cognome << " - " << contattoTrovato->numTelefono << endl << endl;
    } else {
        cout << "[FAIL] Contatto non trovato." << endl << endl;
    }
    ...
    cout << endl << "Rubrica Filtrata: " << endl;
    cout << "----------------------------------" << endl;
